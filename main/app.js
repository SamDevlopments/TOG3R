(function () {
  ...
  if (window.location.protocol === 'file:') {
    ...
    setTimeout(() => {
      showToast("Heads up! Google Login needs a local server to work, not just opening the file.");
  ...
  function showStep(stepId) {
    const steps = [stepLogin, stepSignup, stepProfile];
    steps.forEach(step => {
      if (step.id === stepId) {
        step.hidden = false;
  ...
  if (loginForm) {
    loginForm.onsubmit = (e) => {
      ...
      if (!email.value.trim()) {
        shakeField(email.closest(".field"));
        if (typeof Sam !== "undefined") {
          Sam.onError("email");
        } else {
          mascotSpeak("Wait, what's your email? 🥺", "sad", 4000);
        }
  ...
  if (signupForm) {
    signupForm.onsubmit = (e) => {
      ...
      const rePassword = signupForm.querySelector('input[name="rePassword"]');
      ...
      if (!email.value.trim()) {
        ...
        if (typeof Sam !== "undefined") {
          ...
          mascotSpeak("I need your email to start the roast! 📧", "sad", 4000);
        }
        showToast("Email is required.");
        email.focus();
        return;
      ...
      if (rePassword && rePassword.value !== password.value) {
        shakeField(rePassword.closest(".field"));
        if (typeof Sam !== "undefined") {
          Sam.onError("rePassword");
        } else {
          mascotSpeak("Those passwords don't match, genius! 🙄", "sad", 4000);
        }
        ...
        auth.signInWithPopup(provider)
          ...
          .catch((error) => {
            ...
            if (error.code === 'auth/popup-closed-by-user') {
              ...
              showToast("Google login is not enabled in your Firebase console!");
              if (typeof Sam !== "undefined") {
                Sam.speak("You didn't even enable Google Login in your Firebase console. Typical NPC behavior. 🙄", "rage", 6000);
              }
            } else {
              showToast(error.message);
              if (typeof Sam !== "undefined") Sam.onError("email");
