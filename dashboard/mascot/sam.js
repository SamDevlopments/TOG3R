  ...
  const Sam = {
    ...
    init(elements) {
      ...
      const handleGlobalMouseMove = (e) => {
        ...
        const hoverTarget = e.target && e.target.closest('.movie-card, .card-img, .notifications-container, .notif-dropdown, .verify-notification');
        if (hoverTarget) {
          if (!isIdle) {
            this.onIdle();
          }
    ...
    updateFlight() {
      ...
      if (isFlying && !isRestricted) {
        if (isIdle) {
          ...
          const baseX = mascotRect.left - flyX + (mascotRect.width / 2);
          const baseY = mascotRect.top - flyY + (mascotRect.height / 2);
          const targetX = lastMouseX - baseX;
          const targetY = lastMouseY - baseY;
          
          // Maintain a minimum distance from the cursor
          const dx = targetX - flyX;
          const dy = targetY - flyY;
          const dist = Math.hypot(dx, dy);
          const minDistance = this.params.cursorDistance || 100;
          
          if (dist > 0.1) {
            flyTargetX = (targetX - (dx / dist) * minDistance) * this.params.speed;
            flyTargetY = (targetY - (dy / dist) * minDistance) * this.params.speed;
          } else {
            // Default offset if cursor is exactly on top of Sam
            flyTargetX = (targetX - minDistance) * this.params.speed;
      ...
      const baseX = mascotRect.left - flyX + halfWidth;
      const baseY = mascotRect.top - flyY + halfHeight;
      
      const minX = parentRect.left + margin + halfWidth - baseX;
      const maxX = parentRect.right - margin - halfWidth - baseX;
      const minY = parentRect.top + margin + halfHeight - baseY;
      const maxY = parentRect.bottom - margin - halfHeight - baseY;
      ...
      if (notifDropdown && notifDropdown.classList.contains('active')) {
        const notifRect = notifDropdown.getBoundingClientRect();
        const safeDistance = 75; // px distance to keep away
        
        const targetScreenX = baseX + flyTargetX;
        const targetScreenY = baseY + flyTargetY;
        
        const expandedLeft = notifRect.left - safeDistance - halfWidth;
        const expandedRight = notifRect.right + safeDistance + halfWidth;
        const expandedTop = notifRect.top - safeDistance - halfHeight;
        const expandedBottom = notifRect.bottom + safeDistance + halfHeight;
        
        // If target is inside the "no-fly zone"
        if (targetScreenX > expandedLeft && targetScreenX < expandedRight && 
            targetScreenY > expandedTop && targetScreenY < expandedBottom) {
            
            const distLeft = targetScreenX - expandedLeft;
            const distRight = expandedRight - targetScreenX;
            const distTop = targetScreenY - expandedTop;
            const distBottom = expandedBottom - targetScreenY;
            
            // Find valid escape routes that don't push him off screen
            let escapes = [];
            if (expandedLeft - baseX >= minX) escapes.push({ dist: distLeft, x: expandedLeft - baseX, y: flyTargetY });
            if (expandedRight - baseX <= maxX) escapes.push({ dist: distRight, x: expandedRight - baseX, y: flyTargetY });
            if (expandedTop - baseY >= minY) escapes.push({ dist: distTop, x: flyTargetX, y: expandedTop - baseY });
            if (expandedBottom - baseY <= maxY) escapes.push({ dist: distBottom, x: flyTargetX, y: expandedBottom - baseY });
            
            if (escapes.length > 0) {
                // Take the shortest valid escape route
                escapes.sort((a, b) => a.dist - b.dist);
                flyTargetX = escapes[0].x;
                flyTargetY = escapes[0].y;
            }
        }
      }
    ...
    /**
     * Clears any active speak timeouts or multi-part sequential timeouts
     */
    clearSpeechTimeouts() {
      if (speakTimeout) {
      ...
      if (multiPartTimeouts && multiPartTimeouts.length > 0) {
        multiPartTimeouts.forEach(t => clearTimeout(t));
        multiPartTimeouts = [];
      }
    },

    /**
     * Splits large text responses into 3 neat sequential pieces (Part 1, Part 2, Part 3)
     */
    splitIntoThreePieces(rawText) {
      if (!rawText || typeof rawText !== 'string') return [rawText];
      const text = rawText.trim();

      // If already short and clean, keep as 1 piece
      if (text.length < 65 && text.split(/\s+/).length <= 11) {
        return [text];
      }

      // 1. Break into sentence units
      const sentenceMatches = text.match(/[^.!?]+[.!?]+(?:\s+|$)|[^.!?]+$/g);
      let sentences = sentenceMatches ? sentenceMatches.map(s => s.trim()).filter(Boolean) : [];
      ...
      if (sentences.length > 3) {
        ...
        const part1 = sentences.slice(0, size1).join(' ');
        const part2 = sentences.slice(size1, size1 + size2).join(' ');
        const part3 = sentences.slice(size1 + size2).join(' ');
        return [part1, part2, part3].filter(Boolean);
      }

      // If we have 2 sentences, split the longer one so we get 3 pieces
      if (sentences.length === 2) {
        const s1 = sentences[0];
        const s2 = sentences[1];
        if (s1.length >= s2.length && s1.split(/\s+/).length >= 6) {
          const words1 = s1.split(/\s+/);
          const mid = Math.ceil(words1.length / 2);
          return [
            words1.slice(0, mid).join(' '),
            words1.slice(mid).join(' '),
            s2
          ];
        } else if (s2.split(/\s+/).length >= 6) {
          const words2 = s2.split(/\s+/);
          const mid = Math.ceil(words2.length / 2);
          return [
            s1,
            words2.slice(0, mid).join(' '),
            words2.slice(mid).join(' ')
          ];
        }
      }

      // If 1 long sentence or continuous words, split words into 3 equal pieces
      const words = text.split(/\s+/);
      if (words.length >= 6) {
        const totalWords = words.length;
        const w1 = Math.ceil(totalWords / 3);
        const w2 = Math.ceil((totalWords - w1) / 2);

        const part1 = words.slice(0, w1).join(' ');
        const part2 = words.slice(w1, w1 + w2).join(' ');
        const part3 = words.slice(w1 + w2).join(' ');
        return [part1, part2, part3].filter(Boolean);
      }
    ...
    /**
     * Determines mood from tone or fallback pool
     */
    detectMoodForText(text, defaultMood = "") {
    ...
    /**
     * Makes Sam speak. When response is large, breaks into three pieces:
     * 1. A first piece shows first.
     * 2. Second piece shows in the message area.
     * 3. Third piece shows on the mascot message.
     */
    speak(text, mood = "", duration = 0) {
      ...
      const pieces = this.splitIntoThreePieces(text);

      const renderBubble = (pieceText, pieceMood) => {
        this.setMood(pieceMood);
        if (textEl) {
          textEl.textContent = pieceText;
        }
        if (bubbleEl) {
          bubbleEl.classList.remove("pop");
          void bubbleEl.offsetWidth; // Force reflow for pop animation
      ...
      if (pieces.length <= 1) {
        ...
        const finalMood = this.detectMoodForText(pieces[0], mood);
        renderBubble(pieces[0], finalMood);

        const actualDuration = duration > 0 ? duration : Math.max(3500, Math.min(6000, pieces[0].length * 55 + 1800));
        ...
        // Multi-piece sequence (3 pieces)
        let cumulativeDelay = 0;

        pieces.forEach((piece, index) => {
          const pieceMood = this.detectMoodForText(piece, mood);
          const isFirst = index === 0;
          const isLast = index === pieces.length - 1;

          // Comfortable reading time per piece
          const pieceDisplayDuration = duration > 0
            ? Math.max(2800, Math.floor(duration / pieces.length))
            : Math.max(3000, Math.min(4500, piece.length * 55 + 1600));

          if (isFirst) {
            renderBubble(piece, pieceMood);
          } else {
            const tId = setTimeout(() => {
              renderBubble(piece, pieceMood);
            }, cumulativeDelay);
            multiPartTimeouts.push(tId);
          }

          cumulativeDelay += pieceDisplayDuration;
    ...
    */
    ...
    /**
     * Video player opened: Mascot waves goodbye, leaves the screen
     */
    onVideoOpen() {
      if (!mascotContainer) return;
      this.stopFlying();
      this.speak("Bye-bye! Enjoy your video, I'll be back in a minute! (´｡• ᵕ •｡`) ♡", "happy", 3000);
      
      setTimeout(() => {
        if (mascotContainer) {
          mascotContainer.classList.remove("video-returning");
          mascotContainer.classList.add("video-departing");
          setTimeout(() => {
            if (mascotContainer) {
              mascotContainer.classList.remove("video-departing");
              mascotContainer.classList.add("video-hidden");
            }
          }, 1100);
        }
      }, 1400);
    ...
    /**
     * Video closed / returned home: Mascot flies in from opposite side to greet the user
     */
    onVideoClose() {
      if (!mascotContainer) return;
      mascotContainer.classList.remove("video-hidden", "video-departing");
      mascotContainer.classList.add("video-returning");
      this.speak("Hi! Hope you enjoyed the video! (★ω★)", "hype", 4500);
