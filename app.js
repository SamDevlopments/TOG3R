(function () {
  ...
  // Initialize Firebase if the config is provided
  let auth = null;
  let db = null;
  if (typeof firebase !== "undefined" && firebaseConfig.apiKey !== "YOUR_API_KEY") {
    ...
    console.warn("[Firebase] SDK not loaded or config missing. Running in simulation mode.");
  }

  // --- Check Protocol ---
  if (window.location.protocol === 'file:') {
    console.error("[Firebase] Google Login (signInWithPopup) will NOT work when opening the file directly (file://). Please use a local server (e.g., Live Server in VS Code).");
    setTimeout(() => {
      if (typeof Sam !== "undefined") {
        Sam.speak("You're running this as a file, genius. Google Login needs a real server. Use Live Server! 🙄", "bored", 6000);
      }
    }, 2000);
  }
  ...
  // Auth Navigation
  const useEmailPhone = document.getElementById("useEmailPhone");
  const goToSignup = document.getElementById("goToSignup");
  const goToLogin = document.getElementById("goToLogin");
  const backButtons = document.querySelectorAll(".btn-back");

  function showStep(stepId) {
    const steps = [stepEntry, stepLogin, stepSignup, stepProfile];
    steps.forEach(step => {
  ...
  if (useEmailPhone) {
    useEmailPhone.onclick = () => showStep("stepLogin");
  }
  if (goToSignup) {
    goToSignup.onclick = () => showStep("stepSignup");
  }
  if (goToLogin) {
    goToLogin.onclick = () => showStep("stepLogin");
  }
  ...
  // Helper to handle authenticated user: if profile exists in cloud, redirect to dashboard; if new, continue profile creation
  function handleUserAuthenticated(user, isExplicitLogin = false) {
    if (!user) return;
    console.log("[Auth] Handling authenticated user:", user.email || user.uid);
    sessionStorage.removeItem("tog3r_logged_out");

    const banner = document.getElementById("verificationBanner");
    if (banner) {
      if (!user.emailVerified && !user.isAnonymous && user.providerData && user.providerData.some(p => p.providerId === 'password')) {
    ...
    if (db) {
      db.collection("users").doc(user.uid).get().then((doc) => {
        if (doc.exists) {
          const data = doc.data();
          const hasCompleteData = data.hasCompletedProfile || (data.name && data.handle);
          ...
          if (hasCompleteData) {
            ...
            localStorage.setItem("tog3r_user_profile", JSON.stringify(profileData));
            sessionStorage.removeItem("tog3r_logged_out");

            if (typeof Sam !== "undefined") {
              Sam.speak(`Welcome back, ${data.name || 'friend'}! Loading your dashboard... 🚀`, "happy", 3000);
            }
        ...
        // Account is authenticated, but profile is not completed yet -> continue the process of making an account
        if (user.photoURL) {
          renderGooglePfpPreview(user.photoURL);
        }
        if (user.displayName && !nameInput.value) {
          nameInput.value = user.displayName;
          nameInput.dispatchEvent(new Event("input"));
        }
        if (user.email && !handleInput.value) {
          const suggestedHandle = user.email.split('@')[0].replace(/[^a-zA-Z0-9._]/g, '').slice(0, 10);
          if (suggestedHandle) {
        ...
        if (typeof Sam !== "undefined") {
          Sam.speak(`Account ready! Tell us a little about you to finish your setup. 😊`, "happy", 5000);
        }
  ...
  if (loginForm) {
    loginForm.onsubmit = (e) => {
      ...
      if (!identifier.value.trim()) {
        shakeField(identifier.closest(".field"));
        if (typeof Sam !== "undefined") {
          Sam.onError("email");
        } else {
          mascotSpeak("Wait, what's your email or phone? 🥺", "sad", 4000);
        }
        identifier.focus();
        return;
      }
      ...
      if (!password.value.trim()) {
        shakeField(password.closest(".field"));
        if (typeof Sam !== "undefined") {
          Sam.onError("password");
        } else {
          mascotSpeak("Password? Don't leave me in the dark! 🤫", "sad", 4000);
        }
        password.focus();
        return;
      }
      ...
      if (auth) {
        const loginBtn = loginForm.querySelector(".btn-submit") || submitBtn;
        ...
        const isPhone = /^\+?[0-9]{7,15}$/.test(idVal);
        
        if (isPhone) {
          showToast("Phone auth requires verification. Please use Email or Google login.");
        ...
        auth.setPersistence(persistence)
          .then(() => {
            return auth.signInWithEmailAndPassword(idVal, password.value);
          })
          .then((cred) => {
            if (loginBtn) loginBtn.classList.remove("loading");
            handleUserAuthenticated(cred.user, true);
          })
          .catch(async (error) => {
            if (loginBtn) loginBtn.classList.remove("loading");
            console.warn("[Auth] Login error code:", error.code);

            // If account doesn't exist for this email, automatically offer to continue making an account
            if (error.code === 'auth/user-not-found') {
              if (typeof Sam !== "undefined") {
                Sam.speak("No account found with this email. Let's create one for you! ✨", "think", 6000);
              } else {
                showToast("No account found with this email. Let's create one!");
              }
              const signupEmailInput = signupForm ? signupForm.querySelector('input[name="identifier"]') : null;
              if (signupEmailInput) signupEmailInput.value = idVal;
              showStep("stepSignup");
              return;
            }
            ...
            if (error.code === 'auth/invalid-credential' || error.code === 'auth/wrong-password') {
              try {
                const methods = await auth.fetchSignInMethodsForEmail(idVal);
                if (methods && methods.length === 0) {
                  if (typeof Sam !== "undefined") {
                    Sam.speak("No account found with this email. Let's create one for you! ✨", "happy", 6000);
                  } else {
                    showToast("No account found with this email. Let's create one!");
                  }
                  const signupEmailInput = signupForm ? signupForm.querySelector('input[name="identifier"]') : null;
              ...
              showToast("Incorrect password. Please check your password or reset it.");
              if (typeof Sam !== "undefined") Sam.onError("password");
              return;
            }

            showToast(error.message);
            if (typeof Sam !== "undefined") Sam.onError("email");
          });
      } else {
        // Simulate login success if Firebase is not configured
        showStep("stepProfile");
      }
    };
  }
  ...
  if (signupForm) {
    signupForm.onsubmit = (e) => {
      e.preventDefault();
      const identifier = signupForm.querySelector('input[name="identifier"]');
      ...
      if (!identifier.value.trim()) {
        ...
        if (typeof Sam !== "undefined") {
          ...
          mascotSpeak("I need your email or phone to start the roast! 📧", "sad", 4000);
        }
        identifier.focus();
        return;
      }
      ...
      if (!password.value.trim()) {
        ...
        if (typeof Sam !== "undefined") {
          ...
          mascotSpeak("A secret password, please! 🤫", "sad", 4000);
        }
        password.focus();
        return;
      }
      ...
      if (password.value.length < 8) {
        ...
        if (typeof Sam !== "undefined") {
          ...
          mascotSpeak("Password must be at least 8 characters long. 🔐", "derp", 4000);
        }
        password.focus();
        return;
      }
      ...
      if (auth) {
        ...
        auth.createUserWithEmailAndPassword(idVal, password.value)
          .then((userCredential) => {
            if (signupBtn) signupBtn.classList.remove("loading");
            userCredential.user.sendEmailVerification().catch(() => {});
            
            if (typeof Sam !== "undefined") {
              Sam.speak("Account created! Let's finish your profile details. 🎉", "happy", 5000);
            }
            handleUserAuthenticated(userCredential.user, true);
          })
          .catch((error) => {
            if (signupBtn) signupBtn.classList.remove("loading");
            if (error.code === 'auth/email-already-in-use') {
              if (typeof Sam !== "undefined") {
                Sam.speak("This email is already registered! Please enter your password to log in. 🔑", "think", 5000);
              } else {
                showToast("Account already exists! Please log in.");
              }
              const loginEmailInput = loginForm ? loginForm.querySelector('input[name="identifier"]') : null;
              if (loginEmailInput) loginEmailInput.value = idVal;
              showStep("stepLogin");
              return;
            }
            showToast(error.message);
        ...
        // Simulate signup success if Firebase is not configured
  ...
  const verifyNotification = document.getElementById("verifyNotification");
  ...
  function showVerificationNotification() {
    ...
    verifyNotification.hidden = false;
    verifyNotification.removeAttribute("hidden");
    verifyNotification.classList.remove("fade-out");
  ...
  function hideVerificationNotification() {
    if (!verifyNotification) return;
    verifyNotification.classList.add("fade-out");
    setTimeout(() => {
      verifyNotification.hidden = true;
      verifyNotification.setAttribute("hidden", "");
    }, 1000); // Match fade-out animation duration
  }

  if (closeNotify) {
    closeNotify.onclick = () => hideVerificationNotification();
  }

  // Reset Password Notification Logic
  const resetNotification = document.getElementById("resetNotification");
  const closeResetNotify = document.getElementById("closeResetNotify");
  let resetNotifyTimeout;

  function showResetNotification(email) {
    ...
    const emailHint = document.getElementById("resetEmailHint");
    if (emailHint) {
      emailHint.textContent = maskEmail(email);
    }
    
    // Clear any existing timeout
    if (resetNotifyTimeout) clearTimeout(resetNotifyTimeout);
    
    resetNotification.hidden = false;
    resetNotification.removeAttribute("hidden");
    resetNotification.classList.remove("fade-out");
    
    // Auto fade after 30 seconds
    resetNotifyTimeout = setTimeout(() => {
      hideResetNotification();
    }, 30000);
  }

  function hideResetNotification() {
    if (!resetNotification || resetNotification.hidden) return;
    resetNotification.classList.add("fade-out");
    setTimeout(() => {
      resetNotification.hidden = true;
      resetNotification.setAttribute("hidden", "");
    }, 1000);
  }

  if (closeResetNotify) {
    closeResetNotify.onclick = () => hideResetNotification();
  }
  ...
  function showVerificationBanner() {
    const banner = document.getElementById("verificationBanner");
    const emailHint = document.getElementById("bannerEmailHint");
  ...
  if (closeBanner) {
    closeBanner.onclick = () => hideVerificationBanner();
  }
  ...
  googleButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      console.log("[Auth] Google button clicked. Auth state:", !!auth);
      sessionStorage.removeItem("tog3r_logged_out");
      ...
      if (auth) {
        ...
        if (typeof Sam !== "undefined") {
          Sam.speak("Connecting with your Google account... 🚀", "think", 3000);
        }
        ...
        auth.signInWithPopup(provider)
          .then((result) => {
            ...
            handleUserAuthenticated(result.user, true);
          })
          .catch((error) => {
            console.warn("[Auth] Google sign-in info:", error.code, error.message);
            btn.classList.remove("loading");
            if (btnText) btnText.textContent = originalText;
            
            if (error.code === 'auth/popup-closed-by-user') {
              if (typeof Sam !== "undefined") {
                Sam.speak("Popup closed. Click again when you're ready! 😊", "think", 4000);
              }
            } else {
              sessionStorage.removeItem("tog3r_logged_out");
              if (typeof Sam !== "undefined") {
                Sam.speak("Tell us a little about you! 😊", "happy", 5000);
              }
              showStep("stepProfile");
            }
          });
        ...
        sessionStorage.removeItem("tog3r_logged_out");
        if (typeof Sam !== "undefined") {
          Sam.speak("Tell us a little about you! 😊", "happy", 4000);
        }
        showStep("stepProfile");
      }
    });
  ...
  if (btnForgot) {
    btnForgot.onclick = () => {
      ...
      if (!identifier || identifier.includes("+") || !identifier.includes("@")) {
        if (typeof Sam !== "undefined") {
          Sam.speak("I need your email to send a reset link, genius. 🙄", "bored", 5000);
        }
        return;
      }
      ...
      if (auth) {
        auth.sendPasswordResetEmail(identifier)
          .then(() => {
            if (typeof Sam !== "undefined") {
              Sam.speak("Check your inbox. Try not to forget it this time. (¬_¬)", "smug", 6000);
            }
            showResetNotification(identifier);
          })
          .catch(error => {
            showToast(error.message);
          });
        ...
        if (typeof Sam !== "undefined") {
          Sam.speak("Simulated reset link sent! Check your inbox. (¬_¬)", "smug", 6000);
        }
        showResetNotification(identifier);
      }
    };
  ...
  if (btnLogout) {
    btnLogout.onclick = () => {
      ...
      if (auth) {
        auth.signOut().then(() => {
          if (typeof Sam !== "undefined") {
            Sam.speak("Logged out. See you next time! 👋", "happy", 4000);
          }
          showStep("stepEntry");
        }).catch(() => {
          showStep("stepEntry");
  ...
  const btnResend = document.getElementById("resendVerification");
  if (btnResend) {
    btnResend.onclick = () => {
      if (auth.currentUser) {
        auth.currentUser.sendEmailVerification()
          .then(() => {
            if (typeof Sam !== "undefined") {
              Sam.speak("Fine, I've sent it again. Try to find it this time, okay? 🙄", "bored", 5000);
            }
            showVerificationNotification();
          })
  ...
  if (auth) {
    auth.onAuthStateChanged((user) => {
      ...
      if (user && !isExplicitlyLoggedOut) {
        ...
        handleUserAuthenticated(user, false);
      } else {
        console.log("[Auth] No user signed in or logged out.");
        showStep("stepEntry");
      }
    });
  }

  // Initial state for the card - visible immediately
  if (card) {
  ...
  if (typeof Sam !== "undefined") {
    ...
    // Restricted mode for login page - no full flight, just subtle movement
    Sam.setRestricted(true);
  ...
  // Helper to display Google profile picture in preview with low opacity, pink shade, and plus icon on top
  function renderGooglePfpPreview(photoURL) {
    googlePhotoURL = photoURL || "";
    if (!pfpPreview) return;
    
    // If a custom avatar has already been explicitly selected by the user, don't overwrite it
    if (pfpBlob) return;

    if (photoURL) {
      pfpPreview.innerHTML = `
        <img src="${photoURL}" alt="Google Profile" class="google-pfp-bg" referrerpolicy="no-referrer" />
        <div class="google-pfp-shade"></div>
        <svg class="icon icon-plus" viewBox="0 0 24 24" width="28" height="28" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  function applyDateInternal(d) {
    ...
    if (genderField) {
      genderField.classList.add("show-gender");
    }
  ...
  function openBirthdayPicker() {
    ...
    if (typeof Sam !== "undefined") {
      ...
      mascotSpeak("When were you born? Let's calculate your exact time on Earth! ⏳", "happy", 0);
    }
  }
  ...
  function closeBirthdayPicker() {
    ...
    if (selectedISO) {
      const age = calculateDetailedAge(parseISO(selectedISO));
      if (typeof Sam !== "undefined") {
        Sam.onComplete("dob", age);
      } else {
        mascotSpeak(`Aha! ${age.years} years of awesomeness! 🎂`, "happy", 4000);
      }
    } else {
      if (typeof Sam !== "undefined") {
        Sam.speak("", "", 0);
  ...
  ageDoneBtn.addEventListener("click", (e) => {
    ...
    if (d && d <= new Date()) {
      ...
      if (typeof Sam !== "undefined") {
        ...
        mascotSpeak("Hmm, that birth date doesn't look correct! 🧐", "sad", 4000);
      }
  ...
  [dayInput, monthInput, yearInput].forEach(input => {
    input.addEventListener("focus", () => {
      if (typeof Sam !== "undefined") {
        ...
        mascotSpeak("Type your birth date so I can work my magic! ✨", "", 0);
      }
    });
    ...
    input.addEventListener("input", (e) => {
      ...
      // Dynamic calculation while typing
      const d = parseDMY();
      if (typeof Sam !== "undefined") {
        Sam.onInput("dob", d ? d : "");
      } else {
        if (d && d <= new Date()) {
          const age = calculateDetailedAge(d);
          mascotSpeak(`Aha! Calculating... you are ${age.years} years old! 🎂`, "happy", 0);
        } else {
          mascotSpeak("Keep typing your birth date! ✍️", "", 0);
        }
      }
    });
  ...
  function pupilFollow(e) {
    ...
    // Skip manual follow if Sam is in idle movement mode
    if (typeof Sam !== "undefined" && Sam.getState().isIdle) return;
  ...
  function mascotSpeak(text, mood = "", duration = 0) {
    if (typeof Sam !== "undefined") {
      Sam.speak(text, mood, duration);
      return;
    }
    ...
    if (!text) {
      mascotBubble.classList.remove("show", "pop");
      setMood("");
      return;
    }
    ...
    // Trigger bubble-pop animation by removing and adding "pop"
    mascotBubble.classList.remove("pop");
    void mascotBubble.offsetWidth; // Force reflow
    mascotBubble.classList.add("show", "pop");
  ...
  function showToast(msg) {
    ...
    // Instead of a popup, we let Sam handle the response
    if (typeof Sam !== "undefined") {
      // If the message is already an error/success Sam handles, he might already be speaking.
      // But for generic messages, we trigger a speak.
      Sam.speak(msg, "happy", 5000);
    }
  }
  ...
  [nameInput, handleInput, genderInput].forEach((input) => {
    input.addEventListener("focus", () => {
      setMood("");
      const fieldId = input === nameInput ? "name" : (input === handleInput ? "handle" : "gender");
      if (typeof Sam !== "undefined") {
        Sam.onFocus(fieldId, input.value);
      } else {
        if (input === nameInput) {
          const val = nameInput.value.trim();
          if (val) {
            mascotSpeak(`Hi ${val}! What a wonderful name! ✨`, "happy", 0);
          } else {
            mascotSpeak("What should we call you? 😊", "", 0);
          }
        } else if (input === handleInput) {
          const val = handleInput.value.trim();
          if (val) {
            mascotSpeak(`@${val} sounds like a cool nickname! 😎`, "happy", 0);
          } else {
            mascotSpeak("Choose a unique nickname! No spaces. 🤫", "", 0);
          }
        } else if (input === genderInput) {
          if (genderInput.value) {
            mascotSpeak("Looking good! You can always change this later.", "happy", 0);
          } else {
            mascotSpeak("Select your gender identity! 🌈", "", 0);
          }
        }
      }
    });
    ...
    input.addEventListener("blur", () => {
      const fieldId = input === nameInput ? "name" : (input === handleInput ? "handle" : "gender");
      if (typeof Sam !== "undefined") {
        Sam.onComplete(fieldId, input.value);
      } else {
        mascotSpeak("", "", 1500);
      }
    });
  ...
  nameInput.addEventListener("input", () => {
    ...
    const val = nameInput.value.trim();
    if (typeof Sam !== "undefined") {
      Sam.onInput("name", nameInput.value);
    } else {
      if (val) {
        mascotSpeak(`Hi ${val}! That looks awesome! ✨`, "happy", 0);
      } else {
        mascotSpeak("Type your beautiful name!", "", 0);
      }
    }
    updateTopRowLayout();
  });
  ...
  handleInput.addEventListener("input", () => {
    ...
    handleCounter.textContent = `${handleInput.value.length} / 10`;
    const val = handleInput.value.trim();
    if (typeof Sam !== "undefined") {
      Sam.onInput("handle", handleInput.value);
    } else {
      if (val) {
        mascotSpeak(`@${val} - I like it! 😎`, "happy", 0);
      } else {
        mascotSpeak("Choose a unique nickname! No spaces. 🤫", "", 0);
      }
    }
  ...
  genderInput.addEventListener("change", () => {
    ...
    if (typeof Sam !== "undefined") {
      Sam.onComplete("gender", val);
    } else {
      if (val) {
        mascotSpeak("Awesome! Choice registered. 💖", "happy", 3000);
      }
    }
  });

  // Handle custom gender option button clicks
  const genderButtons = document.querySelectorAll(".gender-option-btn");
  genderButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      // Clear active class from all other buttons
      ...
      // Trigger focus and change event listeners
      if (typeof Sam !== "undefined") {
        Sam.onFocus("gender", val);
      }
    ...
    btn.addEventListener("mouseenter", () => {
      const val = btn.getAttribute("data-value");
      if (typeof Sam !== "undefined" && typeof Sam.onGenderHover === "function") {
        Sam.onGenderHover(val);
      }
    });
  ...
  const avatarTooltipEl = document.getElementById("avatarCursorTooltip");

  pfpTrigger.addEventListener("mouseenter", (e) => {
    if (avatarTooltipEl) {
      avatarTooltipEl.classList.add("is-active");
      avatarTooltipEl.style.left = `${e.clientX}px`;
      avatarTooltipEl.style.top = `${e.clientY}px`;
    }
    if (typeof Sam !== "undefined") {
      Sam.onPfp("hover");
    } else {
      if (!pfpTrigger.classList.contains("has-image")) {
        mascotSpeak("Click me to customize your avatar (optional)! 🎨", "happy", 0);
      } else {
        mascotSpeak("Want to change your avatar style? 💅", "happy", 0);
      }
    }
  });

  pfpTrigger.addEventListener("mousemove", (e) => {
    if (avatarTooltipEl) {
      avatarTooltipEl.style.left = `${e.clientX}px`;
      avatarTooltipEl.style.top = `${e.clientY}px`;
    }
  ...
  pfpTrigger.addEventListener("mouseleave", () => {
    if (avatarTooltipEl) {
      avatarTooltipEl.classList.remove("is-active");
    }
    if (typeof Sam !== "undefined") {
      Sam.onPfp("hoverLeave");
    } else {
      if (document.activeElement !== pfpTrigger) {
        mascotSpeak("", "", 1000);
      }
    }
  });

  pfpTrigger.addEventListener("focus", () => {
    if (typeof Sam !== "undefined") {
      Sam.onPfp("focus");
    } else {
      if (!pfpTrigger.classList.contains("has-image")) {
        mascotSpeak("Let's pick or design a premium avatar! 🎨", "happy", 0);
      } else {
        mascotSpeak("Looking good! Custom style equipped! 😎", "happy", 0);
      }
    }
  });
  pfpTrigger.addEventListener("blur", () => {
    if (typeof Sam !== "undefined") {
      Sam.speak("", "", 1500);
    } else {
      mascotSpeak("", "", 1500);
  ...
  function openPfpSidebar() {
    ...
    if (typeof Sam !== "undefined") {
      ...
      mascotSpeak("Let's find the perfect avatar! Choose a preset, upload, or design one! 🌟", "happy", 5000);
    }
  }

  function closePfpSidebar(isComplete = false) {
    pfpOpen = false;
    ...
    if (typeof Sam !== "undefined") {
      ...
      mascotSpeak("Looking sharp! Perfect! 😍", "happy", 3000);
    }
  }
  ...
  btnGenerateAi.addEventListener("click", () => {
    ...
    if (typeof Sam !== "undefined") {
      ...
      mascotSpeak("Wow! Look at this custom build! Do you like it? 💖", "happy", 4000);
    }
  });
  ...
  function initPfpGrid() {
    ...
    dsBtn.addEventListener("click", () => {
      ...
      if (typeof Sam !== "undefined") {
        ...
        mascotSpeak("Customizing! Change face, eyes, hair, and colors to make it yours! 🎨", "happy", 4000);
      }
    });
    ...
    aiBtn.addEventListener("click", () => {
      ...
      if (typeof Sam !== "undefined") {
        ...
        mascotSpeak("AI Vibe check! Answer these fun questions for a custom look! 🔮", "happy", 4000);
      }
    });
  ...
  function setPfpPreview(blob) {
    ...
    img.className = "chosen-avatar-img";
    img.alt = "Chosen Avatar";
  ...
  pfpUpload.addEventListener("change", (e) => {
    ...
    editImg.onload = () => {
      ...
      if (typeof Sam !== "undefined") {
        ...
        mascotSpeak("Photo uploaded! Rotate, flip, or adjust brightness and contrast! ⚙️", "", 4000);
      }
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (!name) {
      ...
      if (typeof Sam !== "undefined") {
        ...
        mascotSpeak("Wait, what's your name? 🥺", "sad", 4000);
      }
      nameInput.focus();
      return;
    }
    ...
    if (!handle || !handlePattern.test(handle)) {
      ...
      if (typeof Sam !== "undefined") {
        ...
        mascotSpeak("Username needs to be 2-10 letters, numbers, dots, or underscores! 🧐", "sad", 5000);
      }
      handleInput.focus();
      return;
    }

    let birthVal = dobInput.value || selectedISO;
    if (!birthVal) {
      const d = parseDMY();
      if (d && d <= new Date()) {
        applyDateInternal(d);
        birthVal = dobInput.value;
      }
    }

    if (!birthVal) {
      const defaultDate = new Date(2000, 0, 1);
      applyDateInternal(defaultDate);
      birthVal = dobInput.value;
    }

    // Gender is optional or fallback
    const genderValue = genderInput.value || "prefer-not-to-say";
    ...
    const processFinish = (avatarDataUrl) => {
      const currentUser = auth ? auth.currentUser : null;
      const formattedHandle = handle.startsWith("@") ? handle : `@${handle}`;
      const defaultAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=ff7eb3&color=fff&bold=true`;
      
      const profileData = {
        name: name,
        handle: formattedHandle,
        dob: birthVal || "2000-01-01",
        gender: genderValue,
        avatar: avatarDataUrl || (currentUser && currentUser.photoURL) || defaultAvatar,
      ...
      const handleSuccess = () => {
        ...
        const pretty = (dateDisplay && dateDisplay.textContent) || birthVal;
        if (typeof Sam !== "undefined" && typeof Sam.onSuccess === "function") {
          Sam.onSuccess(name, formattedHandle, pretty);
        } else if (typeof mascotSpeak === "function") {
          mascotSpeak("Yay! Account created successfully! Welcome aboard! 🎉", "happy", 8000);
        }
        if (typeof burstConfetti === "function") burstConfetti();
      ...
      // Safety timeout: Ensure redirection always succeeds within 2.5s even if network is slow
      const safetyTimeout = setTimeout(() => {
        console.warn("[Finish] Proceeding via safety timeout.");
        handleSuccess();
      }, 2500);
    ...
    if (pfpBlob) {
      ...
      reader.onloadend = () => {
        processFinish(reader.result);
      };
      reader.onerror = () => {
        processFinish("");
      };
      reader.readAsDataURL(pfpBlob);
    } else {
      processFinish("");
    }
  });

  // Check if birthdate is already pre-filled (autofill, restoration)
  if (dobInput && dobInput.value && genderField) {
    genderField.classList.add("show-gender");
  }

  // Check if gender is pre-filled and activate the matching button
  if (genderInput && genderInput.value) {
    const matchingBtn = document.querySelector(`.gender-option-btn[data-value="${genderInput.value}"]`);
    if (matchingBtn) {
  ...
  setTimeout(() => {
    if (typeof Sam !== "undefined") {
      ...
      mascotSpeak("Hi! I'm here to help. 😊", "happy", 4500);
    }
  }, 800);
})();
