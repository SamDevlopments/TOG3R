(function () {
  ...
  const firebaseConfig = { 
    apiKey: "AIzaSyBA5j5KApSH6rpn-TNjWWaAN_dKzZHzmKI", 
    authDomain: "samdevs.firebaseapp.com", 
    projectId: "samdevs", 
    storageBucket: "samdevs.firebasestorage.app", 
    messagingSenderId: "419358344126", 
    appId: "1:419358344126:web:f53beb35a796ed65662fef", 
    measurementId: "G-H2N208DL1G" 
  };
  ...
  if (loginForm) {
    loginForm.onsubmit = (e) => {
      ...
      if (auth) {
        ...
        if (isPhone) {
          showToast("Phone auth requires verification. Please use Email for now or implement Phone Auth flow.");
  ...
  if (signupForm) {
    signupForm.onsubmit = (e) => {
      ...
      if (auth) {
        ...
        if (isPhone) {
          showToast("Phone auth requires verification. Please use Email for now.");
        ...
        auth.createUserWithEmailAndPassword(idVal, password.value)
          .then((userCredential) => {
            ...
            userCredential.user.sendEmailVerification()
              .then(() => {
                if (typeof Sam !== "undefined") {
                  Sam.speak("I've sent a verification link to your email. Go click it so I know you're not a bot. 🤖", "think", 8000);
                }
                
                // Show notification and switch to login
  ...
  googleButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (auth) {
        ...
        if (typeof Sam !== "undefined") {
          Sam.speak("Asking Google if you're actually a real person... 🤖", "think", 3000);
        }
        ...
        auth.signInWithPopup(provider)
          .then((result) => {
            ...
            if (typeof Sam !== "undefined") {
              Sam.speak(`Welcome, ${result.user.displayName}! Logged in via Google.`, "happy", 8000);
            }
          })
          .catch((error) => {
            ...
            if (error.code === 'auth/popup-closed-by-user') {
              if (typeof Sam !== "undefined") {
                Sam.speak("Did you just close the popup? Scared of your own shadow? xd", "derp", 5000);
              }
            } else if (error.code === 'auth/operation-not-allowed' || error.code === 'auth/configuration-not-found') {
              if (typeof Sam !== "undefined") {
                Sam.speak("You didn't enable Google Login in your Firebase console. Typical NPC behavior. 🙄", "rage", 6000);
              }
            } else {
              showToast(error.message);
              if (typeof Sam !== "undefined") Sam.onError("email");
        ...
        console.warn("[Auth] No Firebase auth instance. Simulation mode.");
        showStep("stepProfile");
      }
    });
  });
  ...
  passwordToggles.forEach(toggle => {
    toggle.addEventListener("click", () => {
      ...
      if (input) {
        ...
        // Update eye icon appearance (optional, keeping it simple for now)
  ...
  if (btnLogout) {
    btnLogout.onclick = () => {
      if (auth) {
        auth.signOut().then(() => {
          if (typeof Sam !== "undefined") {
            Sam.speak("Leaving already? I was just getting started with the roasts. xd", "derp", 5000);
          }
        });
      } else {
        showStep("stepEntry");
          ...
          db.collection("users").doc(user.uid).get().then((doc) => {
            if (doc.exists) {
              ...
              if (data.gender) {
                ...
                const btn = document.querySelector(`.gender-option-btn[data-value="${data.gender}"]`);
                if (btn) btn.click();
              }
              if (data.dob) {
                dobInput.value = data.dob;
                dateDisplay.textContent = data.dob;
              ...
              if (typeof Sam !== "undefined") {
                Sam.speak(`Welcome back, ${data.name || 'friend'}! I remembered your settings.`, "happy", 5000);
              }
            } else {
              ...
              if (user.photoURL && !pfpTrigger.classList.contains("has-image")) {
                fetch(user.photoURL)
                  .then(res => res.blob())
                  .then(blob => {
                    setPfpPreview(blob);
                    if (typeof Sam !== "undefined") {
                      Sam.speak(`I grabbed your Google profile pic. You're welcome. (¬_¬)`, "smug", 5000);
                    }
                  })
                  .catch(err => console.error("[Auth] Error fetching Google photo:", err));
              }
            }
          }).catch(err => console.error("[Firestore] Error loading profile:", err));
        }
  ...
  pfpTrigger.addEventListener("mouseenter", () => {
    if (typeof Sam !== "undefined") {
      Sam.onPfp("hover");
    } else {
      if (!pfpTrigger.classList.contains("has-image")) {
        mascotSpeak("Click me to customize your avatar! 🎨", "happy", 0);
      } else {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (!selectedISO || !dobInput.value) {
      ...
      if (typeof Sam !== "undefined") {
        ...
        mascotSpeak("Don't forget your birthday! I need it for the magic! 🎂", "sad", 5000);
      }
      openBirthdayPicker();
      return;
    }
    ...
    if (!genderInput.value) {
      shakeField(genderField);
      if (typeof Sam !== "undefined") {
        Sam.onError("gender");
      } else {
        mascotSpeak("Please select your gender identity! 🌈", "sad", 5000);
      }
    ...
    if (auth && auth.currentUser && db) {
      db.collection("users").doc(auth.currentUser.uid).set(profileData, { merge: true })
        .then(() => {
          ...
          if (typeof Sam !== "undefined") {
            Sam.onSuccess(name, handle, pretty);
          } else {
            mascotSpeak("Yay! Profile saved successfully! You are awesome! 🎉", "happy", 8000);
          }
          burstConfetti();
          setTimeout(() => {
        ...
        .catch(error => {
          ...
          if (typeof Sam !== "undefined") {
            Sam.speak("Ugh, something went wrong while saving. Try again? 🙄", "sad", 5000);
          }
        });
    } else {
      // Fallback for non-auth simulation
      setTimeout(() => {
        submitBtn.classList.remove("loading");
        submitBtn.querySelector(".btn-text").textContent = "Save & continue";
        const pretty = dateDisplay.textContent;
